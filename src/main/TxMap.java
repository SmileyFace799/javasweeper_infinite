...
public class TxMap extends HashMap<String, BufferedImage> {
    ...
    //Accessors
    public BufferedImage getScaled(int size, String name) {return getScaled(size, size, name);}
    public BufferedImage getScaled(int width, int height, String name) {return getScaled(width, height, this.get(name));}
        ...
        for (File f: Objects.requireNonNull(new File(classPath + path).listFiles())) {
            if (f.isDirectory()) {addFiles(path + "/" + f.getName());}
                ...
                if (img != null) {
                    ...
                    System.out.println("Could not find image: \"" + classPath + path + "/" + f.getName() + "\"");
                }
            }
        }
    }
